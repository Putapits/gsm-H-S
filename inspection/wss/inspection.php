<?php
// Inspector WSS - System Inspections (CRUD)
...
<div class="mb-8">
  <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
    <div>
      <h2 class="text-3xl font-bold text-gray-900 dark:text-white">System Inspections (Inspector)</h2>
      <p class="mt-1 text-gray-600 dark:text-gray-400">Manage every wastewater system inspection from intake to completion.</p>
    </div>
    <div class="flex flex-wrap items-center gap-2">
      ...
      <button id="wssi-add" class="inline-flex items-center rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary/40">Add New</button>
    </div>
  </div>
</div>
...
<section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-lg dark:border-slate-700 dark:bg-slate-900/70">
  <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-3">
    <div class="md:col-span-2">
      ...
      <div class="relative">
        <input id="wssi-search" type="text" placeholder="Search name, email, phone, details..." class="w-full rounded-xl border border-gray-300 bg-white px-4 py-2 pr-10 text-sm text-gray-700 shadow-sm focus:border-primary focus:ring-2 focus:ring-primary/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
    ...
    <div>
      ...
      <select id="wssi-filter-status" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 shadow-sm focus:border-primary focus:ring-2 focus:ring-primary/30 dark:border-slate-600 dark:bg-slate-800 dark:text-white">
  ...
  <div class="overflow-x-auto">
    <table class="min-w-full text-left text-sm text-gray-700 dark:text-gray-200" id="wssi-table">
      ...
      <tbody id="wssi-tbody">
        <?php if (empty($rows)): ?>
          <tr><td colspan="9" class="py-6 px-3 text-center text-gray-500 dark:text-gray-400">No inspection requests found.</td></tr>
        <?php else: foreach ($rows as $r): ?>
          <tr class="wssi-row border-b border-gray-100 transition-colors hover:bg-gray-50 dark:border-slate-800 dark:hover:bg-slate-800/60"
              ...
              data-service="System Inspection"
  ...
  <div id="wssi-pagination" class="mt-6 flex items-center justify-end gap-2 text-sm text-gray-600 dark:text-gray-300"></div>
</section>
...
<div id="wssi-view-modal" class="fixed inset-0 z-50 hidden items-center justify-center">
  ...
  <div class="relative max-h-[85vh] w-[92%] max-w-3xl overflow-y-auto rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
    <div class="mb-4 flex items-center justify-between">
      <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Inspection Request Details</h3>
    ...
    <div id="wssi-view-details" class="grid grid-cols-1 gap-4 text-sm text-gray-700 md:grid-cols-2 dark:text-gray-200"></div>
  </div>
  <style>#wssi-view-modal.show{display:flex;}</style>
</div>
...
<div id="wssi-crud-modal" class="fixed inset-0 z-50 hidden items-center justify-center">
  ...
  <div class="relative max-h-[90vh] w-[92%] max-w-3xl overflow-y-auto rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
    <div class="mb-4 flex items-center justify-between">
      <h3 id="wssi-crud-title" class="text-xl font-semibold text-gray-900 dark:text-white">Add Inspection Request</h3>
    ...
    <form id="wssi-crud-form" class="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
  ...
  <style>#wssi-crud-modal.show{display:flex;}</style>
</div>
...
<script>
  ...
  function renderWssiPagination(pages){ if(!wssiPagination)return; wssiPagination.innerHTML=''; const baseBtn = ()=>{ const btn=document.createElement('button'); btn.type='button'; btn.className='px-3 py-1 rounded-lg border text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-primary/30 border-gray-300 bg-white text-gray-600 hover:bg-gray-100 dark:border-slate-600 dark:bg-slate-800 dark:text-gray-200 dark:hover:bg-slate-700'; return btn; };
    ...
    wssiPagination.appendChild(prev); for(let i=1;i<=pages;i++){ const b=baseBtn(); b.textContent=i; if(i===wssiPage){ b.className='px-3 py-1 rounded-lg bg-primary text-white text-sm font-semibold shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/40'; } else { b.onclick=()=>{wssiPage=i;applyWssiFilters();}; }
    wssiPagination.appendChild(b);} const next=baseBtn(); next.textContent='Next'; next.disabled=wssiPage>=pages; if(next.disabled){ next.classList.add('opacity-60','cursor-not-allowed'); } else { next.onclick=()=>{wssiPage++;applyWssiFilters();}; }
  ...
  function openViewModal(row){ const modal=document.getElementById('wssi-view-modal'); const details=document.getElementById('wssi-view-details'); if(!modal||!details||!row)return; details.innerHTML=`
  ...
  function fillFormFromRow(row){ const f=wssiCrudForm; f.elements['id'].value=row.dataset.id||''; f.elements['full_name'].value=row.dataset.name||''; f.elements['email'].value=row.dataset.email||''; f.elements['phone'].value=row.dataset.phone||''; f.elements['address'].value=row.dataset.address||''; f.elements['service_details'].value=row.dataset.details||''; f.elements['preferred_date'].value=row.dataset.preferred_date||''; f.elements['urgency'].value=(row.dataset.urgency||'medium').toLowerCase(); f.elements['status'].value=(row.dataset.status||'pending').toLowerCase(); }
  function openCrudModal(mode,row){ if(!wssiCrudModal)return; wssiCrudForm.reset(); wssiCrudForm.elements['mode'].value=mode; wssiCurrentRow=null; if(mode==='edit'&&row){ wssiCrudTitle.textContent='Edit Inspection Request'; fillFormFromRow(row); wssiCurrentRow=row;} else { wssiCrudTitle.textContent='Add Inspection Request'; wssiCrudForm.elements['status'].value='pending'; wssiCrudForm.elements['urgency'].value='medium'; } wssiCrudModal.classList.add('show'); wssiCrudModal.classList.remove('hidden'); }
  function closeCrudModal(){ if(!wssiCrudModal)return; wssiCrudModal.classList.remove('show'); wssiCrudModal.classList.add('hidden'); }
  ...
  wssiCrudForm?.addEventListener('submit', async (e)=>{ e.preventDefault(); const mode=wssiCrudForm.elements['mode'].value; const id=parseInt(wssiCrudForm.elements['id'].value||'0',10); const payload={}; if(mode==='edit'){ if(!id) return alert('Invalid ID'); payload.action='update'; payload.id=id; ['status','urgency','preferred_date','address','service_details'].forEach(k=>{ const v=wssiCrudForm.elements[k]?.value ?? null; if(v!==null) payload[k]=v; }); } else { payload.action='create'; payload.service_type='<?php echo $serviceType; ?>'; payload.user_email=wssiCrudForm.elements['user_email'].value; payload.full_name=wssiCrudForm.elements['full_name'].value; payload.email=wssiCrudForm.elements['email'].value; payload.phone=wssiCrudForm.elements['phone'].value; payload.address=wssiCrudForm.elements['address'].value; payload.service_details=wssiCrudForm.elements['service_details'].value; payload.preferred_date=wssiCrudForm.elements['preferred_date'].value; payload.urgency=wssiCrudForm.elements['urgency'].value; } try{ const result=await apiCall(payload); if(!result.success) throw new Error('Operation failed'); if(mode==='edit'){ if(wssiCurrentRow){ const row=wssiCurrentRow; if(payload.preferred_date!==undefined){ row.dataset.preferred_date=payload.preferred_date; const cell=row.children[4]; if(cell) cell.innerText=payload.preferred_date; } if(payload.urgency!==undefined){ row.dataset.urgency=payload.urgency; const cell=row.children[5]; if(cell){ const span=cell.querySelector('span'); if(span){ span.textContent=(payload.urgency||'').toUpperCase(); span.className='px-2 py-1 rounded text-xs font-medium '+urgencyBadgeClass(payload.urgency); } } } if(payload.status!==undefined){ row.dataset.status=payload.status; const cell=row.children[6]; if(cell){ const span=cell.querySelector('span'); if(span){ span.textContent=(payload.status||'').toUpperCase(); span.className='px-2 py-1 rounded text-xs font-medium '+statusBadgeClass(payload.status); } } } if(payload.address!==undefined) row.dataset.address=payload.address; if(payload.service_details!==undefined) row.dataset.details=payload.service_details; }
        wssiCurrentRow=null; closeCrudModal(); applyWssiFilters(); alert('Save Changes Successfully.');
  ...
  if (wssiCrudForm && !wssiCrudForm.dataset.overrideAllFields){
    wssiCrudForm.dataset.overrideAllFields = '1';
    wssiCrudForm.addEventListener('submit', async (e)=>{
      try {
        e.preventDefault();
        ...
        if (mode === 'edit'){
          ...
          ['status','urgency','preferred_date','address','service_details','full_name','email','phone'].forEach(k=>{
            const v = wssiCrudForm.elements[k]?.value ?? null;
          ...
          payload.user_email = wssiCrudForm.elements['user_email'].value;
          payload.full_name = wssiCrudForm.elements['full_name'].value;
          payload.email = wssiCrudForm.elements['email'].value;
          payload.phone = wssiCrudForm.elements['phone'].value;
          payload.address = wssiCrudForm.elements['address'].value;
          payload.service_details = wssiCrudForm.elements['service_details'].value;
          payload.preferred_date = wssiCrudForm.elements['preferred_date'].value;
          payload.urgency = wssiCrudForm.elements['urgency'].value;
        }
        const result = await apiCall(payload);
        if (!result.success) throw new Error('Operation failed');
        if (mode === 'edit'){
          if (wssiCurrentRow){
            const row = wssiCurrentRow;
