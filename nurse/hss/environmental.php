<?php
// Nurse HSS - Environmental Monitoring (CRUD)
...
try {
  ...
  error_log('Nurse HSS environmental fetch error: ' . $e->getMessage());
  $rows = [];
}
...
<section class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
  <div class="flex flex-col gap-2">
    ...
    <h2 class="text-3xl font-bold text-gray-900 dark:text-white">Environmental Monitoring</h2>
    <p class="text-gray-600 dark:text-gray-400">Manage environmental monitoring submissions.</p>
  </div>
  <div class="flex flex-wrap items-center gap-2">
    <button id="nhsse-env-export" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-600 transition hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-primary/30 dark:border-slate-600 dark:bg-slate-800 dark:text-gray-200 dark:hover:bg-slate-700">Export CSV</button>
    <button id="nhsse-env-add" class="inline-flex items-center rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary/40">Add Submission</button>
  </div>
</section>
...
<section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
  <div class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
    <div class="md:col-span-2">
      ...
      <div class="relative">
        <input id="nhsse-env-search" type="text" placeholder="Search name, email, phone, details..." class="w-full rounded-lg border border-gray-300 bg-gray-50 px-4 py-2 pr-12 text-sm text-gray-700 shadow-sm transition placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30 dark:border-slate-600 dark:bg-slate-800 dark:text-gray-100 dark:placeholder-gray-500">
    ...
    <div>
      ...
      <select id="nhsse-env-filter-status" class="w-full rounded-lg border border-gray-300 bg-gray-50 px-3 py-2 text-sm text-gray-700 shadow-sm transition focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30 dark:border-slate-600 dark:bg-slate-800 dark:text-gray-100">
  ...
  <div class="overflow-x-auto">
    <table class="min-w-full divide-y divide-gray-100 text-sm text-gray-700 dark:divide-slate-800 dark:text-gray-200" id="nhsse-env-table">
      ...
      <tbody id="nhsse-env-tbody" class="divide-y divide-gray-100 dark:divide-slate-800">
        <?php if (empty($rows)): ?>
        ...
        <?php else: foreach ($rows as $r): ?>
          ...
          ?>
          <tr class="nhsse-env-row border-b border-transparent transition-colors hover:bg-gray-50 dark:hover:bg-slate-800/60"
  ...
  <div id="nhsse-env-pagination" class="mt-4 flex items-center justify-end gap-2 text-xs font-semibold text-gray-600 dark:text-gray-300"></div>
</section>

<!-- View Modal -->
<div id="nhsse-env-view-modal" class="fixed inset-0 hidden items-center justify-center z-50">
  ...
  <div class="modal-panel relative max-h-[85vh] w-[92%] max-w-3xl overflow-y-auto rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
    ...
    <div id="nhsse-env-view-details" class="grid grid-cols-1 gap-4 text-sm text-gray-700 md:grid-cols-2 dark:text-gray-200"></div>
  </div>
  <style>#nhsse-env-view-modal.show{display:flex;align-items:center;justify-content:center;padding:1.5rem;}</style>
</div>

<!-- CRUD Modal -->
<div id="nhsse-env-crud-modal" class="fixed inset-0 hidden items-center justify.center z-50">
  ...
  <div class="modal-panel relative max-h-[90vh] w-[92%] max-w-3xl overflow-y-auto rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
    <div class="mb-4 flex items-center justify-between">
      <h3 id="nhsse-env-crud-title" class="text-xl font-semibold text-gray-900 dark:text-white">Add Submission</h3>
    ...
    <form id="nhsse-env-crud-form" class="grid grid-cols-1 gap-6 text-sm md:grid-cols-2">
  ...
  <style>#nhsse-env-crud-modal.show{display:flex;align-items:center;justify-content:center;padding:1.5rem;}</style>
</div>
...
<script>
  const nenvSearch = document.getElementById('nhsse-env-search');
  const nenvBody = document.getElementById('nhsse-env-tbody');
  const nenvFilterStatus = document.getElementById('nhsse-env-filter-status');
  const nenvTable = document.getElementById('nhsse-env-table');
  const nenvExport = document.getElementById('nhsse-env-export');
  const nenvPagination = document.getElementById('nhsse-env-pagination');
  const nenvAdd = document.getElementById('nhsse-env-add');
  let nenvSortKey='id', nenvSortAsc=false, nenvPage=1; const nenvPageSize=10;

  function nenvRows(){ return Array.from(nenvBody?.querySelectorAll('.nhsse-env-row')||[]); }
  function applyNenvFilters(){
    const q=(nenvSearch?.value||'').toLowerCase(); const fs=(nenvFilterStatus?.value||'').toLowerCase();
    let rows=nenvRows();
    ...
    rows.sort((a,b)=>{ const ka=(a.dataset[nenvSortKey]||'').toLowerCase(); const kb=(b.dataset[nenvSortKey]||'').toLowerCase(); if(ka<kb)return nenvSortAsc?-1:1; if(ka>kb)return nenvSortAsc?1:-1; return 0; });
    const total=rows.length; const pages=Math.max(1, Math.ceil(total/nenvPageSize)); if(nenvPage>pages) nenvPage=pages; const start=(nenvPage-1)*nenvPageSize; const visible=new Set(rows.slice(start,start+nenvPageSize));
    nenvRows().forEach(tr=>tr.style.display='none'); visible.forEach(tr=>tr.style.display='');
    renderNenvPagination(pages);
  }
  function renderNenvPagination(pages){
    if(!nenvPagination) return;
    ...
    nenvPagination.innerHTML='';
    const prev=document.createElement('button');
    prev.textContent='Prev';
    prev.className=baseBtn;
    prev.disabled=nenvPage<=1;
    if(prev.disabled) prev.classList.add('opacity-50','cursor-not-allowed');
    prev.onclick=()=>{nenvPage--;applyNenvFilters();};
    nenvPagination.appendChild(prev);
    for(let i=1;i<=pages;i++){
      const b=document.createElement('button');
      b.textContent=i;
      b.className=(i===nenvPage?activeBtn:baseBtn);
      b.onclick=()=>{nenvPage=i;applyNenvFilters();};
      nenvPagination.appendChild(b);
    }
    const next=document.createElement('button');
    next.textContent='Next';
    next.className=baseBtn;
    next.disabled=nenvPage>=pages;
    if(next.disabled) next.classList.add('opacity-50','cursor-not-allowed');
    next.onclick=()=>{nenvPage++;applyNenvFilters();};
    nenvPagination.appendChild(next);
  }
  [nenvSearch, nenvFilterStatus].forEach(el=>{ if(el) el.addEventListener('input', applyNenvFilters); if(el) el.addEventListener('change', applyNenvFilters); });
  if (nenvTable){ nenvTable.querySelectorAll('th.sortable').forEach(th=>{ th.addEventListener('click',()=>{ const key=th.getAttribute('data-key'); if(nenvSortKey===key){nenvSortAsc=!nenvSortAsc;} else {nenvSortKey=key; nenvSortAsc=true;} applyNenvFilters(); }); }); }
  ...
  function nenvToCSV(){ const headers=['ID','Full Name','Email','Phone','Preferred Date','Urgency','Status','Created']; const rows=nenvRows().filter(tr=>tr.style.display!=='none'); const lines=[headers.join(',')]; rows.forEach(tr=>{ const cells=Array.from(tr.children).slice(0,8).map(td=>'"'+(td.innerText||'').replace(/"/g,'\\"')+'"'); lines.push(cells.join(',')); }); return lines.join('\n'); }
  if (nenvExport) nenvExport.addEventListener('click', ()=>download('hss_environmental.csv', nenvToCSV()));
  ...
  function openViewModal(row){ const modal=document.getElementById('nhsse-env-view-modal'); const details=document.getElementById('nhsse-env-view-details'); if(!modal||!details||!row)return; details.innerHTML=`
  ...
  function closeViewModal(){ const modal=document.getElementById('nhsse-env-view-modal'); if(!modal)return; modal.classList.remove('show'); modal.classList.add('hidden'); }
  window.openViewModal=openViewModal; window.closeViewModal=closeViewModal;

  const nenvCrudModal=document.getElementById('nhsse-env-crud-modal'); const nenvCrudForm=document.getElementById('nhsse-env-crud-form'); const nenvCrudTitle=document.getElementById('nhsse-env-crud-title');
  function fillFormFromRow(row){ const f=nenvCrudForm; f.elements['id'].value=row.dataset.id||''; f.elements['full_name'].value=row.dataset.name||''; f.elements['email'].value=row.dataset.email||''; f.elements['phone'].value=row.dataset.phone||''; f.elements['address'].value=row.dataset.address||''; f.elements['service_details'].value=row.dataset.details||''; f.elements['preferred_date'].value=row.dataset.preferred_date||''; f.elements['urgency'].value=(row.dataset.urgency||'medium').toLowerCase(); f.elements['status'].value=(row.dataset.status||'pending').toLowerCase(); }
  function openCrudModal(mode,row){ if(!nenvCrudModal)return; nenvCrudForm.reset(); nenvCrudForm.elements['mode'].value=mode; if(mode==='edit'&&row){ nenvCrudTitle.textContent='Edit Submission'; fillFormFromRow(row);} else { nenvCrudTitle.textContent='Add Submission'; nenvCrudForm.elements['status'].value='pending'; nenvCrudForm.elements['urgency'].value='medium'; } nenvCrudModal.classList.add('show'); nenvCrudModal.classList.remove('hidden'); }
  function closeCrudModal(){ if(!nenvCrudModal)return; nenvCrudModal.classList.remove('show'); nenvCrudModal.classList.add('hidden'); }
  window.openCrudModal=openCrudModal; window.closeCrudModal=closeCrudModal; if(nenvAdd) nenvAdd.addEventListener('click',()=>openCrudModal('create'));
  ...
  async function deleteRow(row){ if(!row) return; const id=parseInt(row.dataset.id||'0',10); if(!id) return alert('Invalid ID'); if(!confirm('Delete this submission?')) return; try{ const result=await apiCall({action:'delete', id}); if(result.success){ row.remove(); applyNenvFilters(); } else alert('Delete failed.'); }catch(e){ console.error(e); alert('Server error.'); } }
  window.deleteRow=deleteRow;

  nenvCrudForm?.addEventListener('submit', async (e)=>{ e.preventDefault(); const mode=nenvCrudForm.elements['mode'].value; const id=parseInt(nenvCrudForm.elements['id'].value||'0',10); const payload={}; if(mode==='edit'){ if(!id) return alert('Invalid ID'); payload.action='update'; payload.id=id; ['status','urgency','preferred_date','address','service_details','full_name','email','phone'].forEach(k=>{ const v=nenvCrudForm.elements[k]?.value ?? null; if(v!==null) payload[k]=v; }); } else { payload.action='create'; payload.service_type='<?php echo $serviceType; ?>'; payload.user_email=nenvCrudForm.elements['user_email'].value; payload.full_name=nenvCrudForm.elements['full_name'].value; payload.email=nenvCrudForm.elements['email'].value; payload.phone=nenvCrudForm.elements['phone'].value; payload.address=nenvCrudForm.elements['address'].value; payload.service_details=nenvCrudForm.elements['service_details'].value; payload.preferred_date=nenvCrudForm.elements['preferred_date'].value; payload.urgency=nenvCrudForm.elements['urgency'].value; } try{ const result=await apiCall(payload); if(!result.success) throw new Error(JSON.stringify(result)); alert('Saved successfully'); location.reload(); } catch(err){ console.error(err); let msg='Error saving data.'; try{ const j=JSON.parse(err.message); if(j&&j.message) msg=j.message; }catch{} alert(msg); } });
