...
$allowedTypes = ['disease-monitoring','environmental-monitoring'];
...
try {
  switch ($action) {
    case 'update': {
      ...
      $sql = 'UPDATE service_requests SET '.implode(', ', $fields).', updated_at = CURRENT_TIMESTAMP WHERE id = :id AND service_type IN (\'disease-monitoring\',\'environmental-monitoring\') AND deleted_at IS NULL';
      $stmt = $db->prepare($sql);
    ...
    case 'delete': {
      $id = (int)($input['id'] ?? 0);
      if ($id <= 0) jerr('Invalid ID');
      $stmt = $db->prepare('UPDATE service_requests SET deleted_at = CURRENT_TIMESTAMP WHERE id = :id AND service_type IN (\'disease-monitoring\',\'environmental-monitoring\') AND deleted_at IS NULL');
