...
$allowedTypes = ['vaccination','nutrition-monitoring'];
...
try {
  switch ($action) {
    case 'update': {
      ...
      $statusAllowed = ['pending','in_progress','completed','cancelled'];
      $urgencyAllowed = ['low','medium','high','emergency'];
      foreach ($allowed as $k) {
        if (array_key_exists($k, $input)) {
          ...
          if ($k === 'urgency' && $v !== null && $v !== '') {
            $v = strtolower((string)$v);
            if (!in_array($v, $urgencyAllowed, true)) jerr('Invalid urgency');
          }
          if ($k === 'preferred_date' && ($v === '' || $v === null)) { $v = null; }
          $fields[] = "$k = :$k";
          $params[":$k"] = $v;
        }
      }
      if (empty($fields)) jerr('No fields to update');
      $sql = 'UPDATE service_requests SET '.implode(', ', $fields).', updated_at = CURRENT_TIMESTAMP WHERE id = :id AND service_type IN (\'vaccination\',\'nutrition-monitoring\') AND deleted_at IS NULL';
      $stmt = $db->prepare($sql);
    ...
    case 'delete': {
      $id = (int)($input['id'] ?? 0);
      if ($id <= 0) jerr('Invalid ID');
      $stmt = $db->prepare('UPDATE service_requests SET deleted_at = CURRENT_TIMESTAMP WHERE id = :id AND service_type IN (\'vaccination\',\'nutrition-monitoring\') AND deleted_at IS NULL');
    ...
    case 'create': {
      ...
      if ($email === '' && $user_email !== '' && filter_var($user_email, FILTER_VALIDATE_EMAIL)) { $email = $user_email; }
      ...
      $urgency = !empty($input['urgency']) ? strtolower(trim((string)$input['urgency'])) : 'medium';
      $urgencyAllowed = ['low','medium','high','emergency'];
      if (!in_array($urgency, $urgencyAllowed, true)) jerr('Invalid urgency');
