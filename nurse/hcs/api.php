...
$allowedTypes = ['medical-consultation','emergency-care','preventive-care'];
...
try {
  switch ($action) {
    case 'update': {
      ...
      $allowed = ['full_name','email','phone','address','service_details','preferred_date','urgency','status'];
      $fields = [];
      $params = [':id'=>$id];
      foreach ($allowed as $k) {
        if (array_key_exists($k, $input)) { $fields[] = "$k = :$k"; $params[":$k"] = $input[$k] === '' ? null : $input[$k]; }
      }
