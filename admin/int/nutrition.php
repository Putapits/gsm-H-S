<?php
// Admin read-only Nutrition Monitoring requests
...
try {
  ...
  error_log('Admin Nutrition fetch error: ' . $e->getMessage());
  $requests = [];
}
...
<div class="mb-8 rounded-2xl p-8 border border-violet-100 shadow-lg bg-gradient-to-r from-violet-50 via-purple-50 to-fuchsia-50 dark:border-slate-700/70 dark:bg-gradient-to-r dark:from-slate-800 dark:via-slate-900 dark:to-slate-950 dark:shadow-[0_15px_35px_-15px_rgba(15,23,42,0.8)]">
  <div class="flex items-center justify-between">
    <div>
      <h2 class="text-2xl font-bold text-gray-900 dark:text-slate-100">Nutrition Tracker</h2>
      <p class="text-gray-600 dark:text-slate-300 mt-1">Read-only nutrition monitoring requests submitted by citizens.</p>
    </div>
    <div class="flex items-center gap-3">
      ...
      <button id="admin-nut-export" class="px-3 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-lg text-xs font-medium shadow-sm">Export CSV</button>
    </div>
  </div>
</div>
...
<section class="bg-white dark:bg-slate-900/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-md">
  <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
    <div class="md:col-span-2">
      ...
      <div class="relative">
        <input id="admin-nut-search" type="text" placeholder="Search name, email, phone, details..." class="bg-white dark:bg-slate-900/50 text-gray-900 dark:text-slate-100 placeholder-gray-500 dark:placeholder-slate-400 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 pr-10 focus:outline-none focus:ring-2 focus:ring-violet-500 dark:focus:ring-violet-400 w-full">
    ...
    <div>
      ...
      <select id="admin-nut-filter-status" class="w-full bg-white dark:bg-slate-900/50 text-gray-900 dark:text-slate-100 rounded-lg px-3 py-2 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-violet-500 dark:focus:ring-violet-400">
  ...
  <div class="overflow-x-auto">
    <table class="min-w-full text-sm" id="admin-nut-table">
      <thead>
      ...
      <tbody id="admin-nut-tbody">
        <?php if (empty($requests)): ?>
          <tr>
            <td colspan="9" class="py-6 px-3 text-center text-gray-500 dark:text-slate-400">No nutrition monitoring requests found.</td>
          </tr>
        <?php else: foreach ($requests as $r): ?>
          <tr class="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/60 admin-nut-row"
            ...
            <td class="py-3 px-3 text-right">
              <button class="px-3 py-1 bg-violet-600 text-white rounded-lg hover:bg-violet-500 text-xs font-medium shadow-sm" onclick="openViewModal(this.closest('tr'))">View</button>
            </td>
          </tr>
  ...
  <div id="admin-nut-pagination" class="mt-4 flex items-center justify-end gap-2 text-sm"></div>
</section>

<!-- View Modal -->
<div id="admin-nut-view-modal" class="fixed inset-0 hidden items-center justify-center z-50">
  ...
  <div class="relative modal-panel bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl max-w-3xl w-[92%] p-6 max-h-[85vh] overflow-y-auto shadow-xl">
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-xl font-semibold text-gray-900 dark:text-slate-100">Nutrition Request Details</h3>
    ...
    <div id="admin-nut-view-details" class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm"></div>
  </div>
  <style>#admin-nut-view-modal.show{display:flex;}</style>
</div>
...
<script>
  const nutSearch = document.getElementById('admin-nut-search');
  const nutBody = document.getElementById('admin-nut-tbody');
  const nutFilterStatus = document.getElementById('admin-nut-filter-status');
  const nutTable = document.getElementById('admin-nut-table');
  const nutExport = document.getElementById('admin-nut-export');
  const nutPagination = document.getElementById('admin-nut-pagination');
  let nutSortKey = 'id'; let nutSortAsc = false; let nutPage = 1; const nutPageSize = 10;

  function nutRows(){ return Array.from(nutBody?.querySelectorAll('.admin-nut-row')||[]); }
  ...
  function renderNutPagination(pages){ if(!nutPagination)return; nutPagination.innerHTML=''; const prev=document.createElement('button'); prev.textContent='Prev'; prev.className='px-2 py-1 bg-gray-700 hover:bg-gray-600 text-white rounded'; prev.disabled=nutPage<=1; prev.onclick=()=>{nutPage--;applyNutFilters();}; nutPagination.appendChild(prev); for(let i=1;i<=pages;i++){ const b=document.createElement('button'); b.textContent=i; b.className='px-2 py-1 rounded '+(i===nutPage?'bg-primary text-white':'bg-gray-700 hover:bg-gray-600 text-white'); b.onclick=()=>{nutPage=i;applyNutFilters();}; nutPagination.appendChild(b);} const next=document.createElement('button'); next.textContent='Next'; next.className='px-2 py-1 bg-gray-700 hover:bg-gray-600 text-white rounded'; next.disabled=nutPage>=pages; next.onclick=()=>{nutPage++;applyNutFilters();}; nutPagination.appendChild(next); }
  ...
  function openViewModal(row){ const modal=document.getElementById('admin-nut-view-modal'); const details=document.getElementById('admin-nut-view-details'); if(!modal||!details||!row)return; details.innerHTML=`
  ...
  function closeViewModal(){ const modal=document.getElementById('admin-nut-view-modal'); if(!modal)return; modal.classList.remove('show'); modal.classList.add('hidden'); }
